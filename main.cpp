...
void database_init(int open_database)
{
    if (open_database)
    {
        ...
        bool flag = httpdeal::initmysql_result(connpool);
        if (!flag)
        {
            exit(-1);
        }
...
int main(int argc, char* argv[])
{
    ...
    while (!stop_server)
    {
        ...
        {
            LOG_ERROR("%s:errno is:%d", "epoll_wait", errno);
            exit(-1);
        }
        ...
        for (int i = 0; i < nready; i++)
        {
            ...
            else
            {
                ...
                {
                    if(!reactors[cfd].dealread())
                    {
                        reactors[cfd].destory();
                        LOG_ERROR("%s:errno is:%d", "read", errno);
                        continue;
                    }
                    pool->addtask(true, reactors+cfd);
                    //reactors[cfd].process();
                ...
                {
                    if(!reactors[cfd].dealwrite())
                    {
                        reactors[cfd].destory();
                        LOG_ERROR("%s:errno is:%d", "write", errno);
